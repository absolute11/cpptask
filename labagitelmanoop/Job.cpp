...
Job::Job(const Job& other) : name(other.name), position(other.position), age(other.age), salary(other.salary) {
}
Job* Job::copy() {
    Job* newJob = new Job();
    newJob->age = this->age;
...
bool Job::equal(const Job& other) const {
    return (name.equal(other.name) && position.equal(other.position) && age == other.age && salary == other.salary);
}